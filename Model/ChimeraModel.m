function sol = ChimeraModel(params,domain,ins)
    ...
    AM = zeros(6,domain + 1);   % Arrivals Matrix
    DM= zeros(6,domain + 1);    % Departure Matrix
    ...
    M(19+j*23, 1) = dot((M(9+j*23, 1)+M(11+j*23, 1)),k_P) + dot(M(7+j*23, 1),k_L);       % T
        ...
        %arraving migration in S_f
        AM([1:3,6],:)=ins.migration([1:3,5],:);
        AM(4,:)=ins.migration(4,:)*(1-delta(1));
        AM(5,:)=ins.migration(4,:)*delta(1);
        AM=round(AM);

        
    for i=1:domain
         %Adding the Arriving population
         M(1,i)=M(1,i)+AM(1,i);
         M(18,i)=M(18,i)+AM(2,i);
         M(4,i)=M(4,i)+AM(3,i);
         M(9,i)=M(9,i)+AM(4,i);
         M(11,i)=M(11,i)+AM(5,i);
         M(7,i)=M(7,i)+AM(6,i);
        ...
        %DEPARTING MIGRATION ON THE DAY i+1
        %departing migration in S_f
        DM(1,i)=binornd(fix(M(1,i+1)),1/ins.migration(6,i));
        %departing migration in R
        DM(2,i)=binornd(fix(M(18,i+1)),1/ins.migration(6,i));
        %departing migration in E^2_f
        DM(3,i)=binornd(fix(M(4,i+1)),1/ins.migration(6,i));
        %departing migration in P^H_f
        DM(4,i)=binornd(fix(M(9,i+1)),1/ins.migration(6,i));
        %departing migration in P^L_f
        DM(5,i)=binornd(fix(M(11,i+1)),1/ins.migration(6,i));
        %departing migration in L
        DM(6,i)=binornd(fix(M(7,i+1)),1/ins.migration(6,i));
        
         %Adding the Arriving population
         M(1,i+1)=M(1,i+1)-DM(1,i);
         M(18,i+1)=M(18,i+1)-DM(2,i);
         M(4,i+1)=M(4,i+1)-DM(3,i);
         M(9,i+1)=M(9,i+1)-DM(4,i);
         M(11,i+1)=M(11,i+1)-DM(5,i);
         M(7,i+1)=M(7,i+1)-DM(6,i);
...
%     tester  
    ...
    M(1:23,:)=M(1:23,:)+M(24:end,:);
    
        outs = [M(1:2,:); sum(M([3, 4],:)); sum(M([5, 6],:)); sum(M([9, 11],:)); sum(M([10, 12],:));...
        M(7:8,:); sum(M([13:16, 22:23],:)); M(17:21,:); sum(M([20, 21],:)); sum(M([7:16, 22:23],:));...
        M(14,:); sum(M([13, 15:16, 22:23],:)); sum(M(13:16,:)); sum(M(22:23,:)); AM ; DM; sum(AM)];

%         outs = [M([1,2]+k*23,:); sum(M([3,4]+k*23,:)); sum(M([5,6]+k*23,:));...
